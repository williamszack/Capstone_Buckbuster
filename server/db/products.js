...
async function addNewProduct({ name, description, price, genre, quantity, image_url, active }) {
	try {
		const {
			rows: [product],
		} = await client.query(
        ...
        (name, description, price, genre, quantity, image_url, active)
        VALUES
        ($1, $2, $3, $4, $5, $6, $7)
        RETURNING *;
        `,
...
async function deactivateProduct(product_id) {
	try {
		const {
			rows: [product],
		} = await client.query(
		...
		console.error("Error with deactivating product", error);
	}
}

async function reactivateProduct(product_id) {
	try {
		const {
			rows: [product],
		} = await client.query(
		...
		console.error("Error with reactivating product", error);
	}
}
